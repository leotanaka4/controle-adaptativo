...
\begin{itemize}
    \item \textbf{Planta:} $\begin{cases}
        P(s) = \dfrac{k_p(s+1)(s+3)}{(s+1)(s+4)(s+7)} \quad com \quad k_p = 2.3816 \quad (n^* = 1) \\ 
        P(s) = \dfrac{k_p(s+5)}{(s+2)(s+4)(s+5)} \quad com \quad k_p = 2 \quad (n^* = 2)
    \end{cases}$
    \item \textbf{Modelo de referência:} $\begin{cases}
        M(s) = \dfrac{k_p(s+2)(s+4)}{(s+1)(s+3)(s+5)} \quad com \quad k_m = 1.8750 \quad (n^* = 1) \\ 
        M(s) = \dfrac{k_p(s+2)}{(s+1)(s+3)(s+5)} \quad com \quad k_m = 7.5 \quad (n^* = 2)
    \end{cases}$
    \item \textbf{Filtro auxiliar:} $\begin{cases}
        \frac{1}{\Lambda(s)} = \dfrac{1}{(s + 2)(s + 4)} \quad (n^* = 1) \\
        \frac{1}{\Lambda(s)} = \dfrac{1}{(s + 1)(s + 2)} \quad (n^* = 2)
    \end{cases}$
    \end{itemize}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_1/base.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_1/0.95.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_1/y0_1.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_1/gamma_100.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_1/add_freq.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_1/add_freq_0.95.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_2/base.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_2/0.95.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_2/y0_1.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_2/gamma_100.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_2/add_freq.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nstar_2/add_freq_0.95.png}
    \caption{Resultados da Simulação \#12}
    \label{fig:sim12}
\end{figure}
...
\textbf{Simulações \#1 e \#7 – Caso Base:} O sistema acompanha com precisão a referência $y_m(t)$, apresentando erro $e_a \rightarrow 0$, mas não convergência dos parâmetros $\theta(t)$. Esses resultados validam a estabilidade uniforme global do esquema, apesar da não convergência dos parâmetros, o que é esperado em sistemas adaptativos diretos uma vez que a convergência depende de excitação persistente.

\textbf{Simulações \#2 e \#8 – $\theta(0) = 0{,}95 \cdot \theta^*$:} A proximidade da estimativa inicial em relação ao valor ideal acelera a convergência dos parâmetros e reduz o transiente. Dessa forma, o controlador mantém bom desempenho em termos de erro de rastreamento, mas ainda não é capaz de eliminar completamente o erro paramétrico $\tilde{\theta}$.

\textbf{Simulações \#3 e \#9 – $y_0 = 10$:} A condição inicial elevada gera um transiente mais acentuado, como esperado, mas o sistema retoma rapidamente o rastreamento da referência. Isso demonstra a robustez do MRAC Direto frente a perturbações iniciais.

\textbf{Simulações \#4 e \#10 – $\Gamma = 100I$:} O aumento do ganho de adaptação para $\Gamma = 100I$ introduziu mais oscilações na convergência dos parâmetros $\theta$, embora tenha diminuído os picos iniciais do erro de rastreamento. Esse ajuste, entretanto, levou a um aumento na magnitude da lei de controle, evidenciando o compromisso entre desempenho inicial e esforço de controle.

\textbf{Simulações \#5 e \#11 – Sinal de Referência Senoidal:} A introdução de uma referência senoidal intensifica a persistência de excitação, favorecendo a atualização dos parâmetros e promovendo melhor convergência paramétrica. Mesmo com maior complexidade do sinal, o erro de rastreamento permanece pequeno, reforçando a eficácia da adaptação.

\textbf{Simulações \#6 e \#12 – $\theta(0) = 0{,}95 \cdot \theta^*$ com Referência Senoidal:} A combinação de uma boa estimativa inicial e um sinal com persistência de excitação acentuada resulta em desempenho superior. A adaptação é rápida e estável, com erro reduzido e transiente mínimo, evidenciando o impacto positivo da sinergia entre esses fatores.

\textbf{Conclusão:} As simulações demonstram as principais propriedades do MRAC Direto: estabilidade garantida sob determinadas condições, rastreamento eficaz da referência e capacidade de adaptação mesmo diante de incertezas ou variações nas condições iniciais. Além disso, a escolha adequada dos parâmetros de adaptação e do sinal de referência permite otimizar o desempenho sem comprometer a estabilidade do sistema.
